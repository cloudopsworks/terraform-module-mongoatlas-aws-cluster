...
#     rotation_lambda_name: "fn-name"  # (Optional) AWS Lambda function name for Secrets Manager rotation
...
#     rotate_immediately: true         # (Optional) Rotate the secret immediately on first creation. Default: true
...
#     export:
#       bucket_name: "my-s3-bucket"   # (Required) S3 bucket name for backup export
#       iam_role_id: "role-id"        # (Required) Atlas IAM role ID for S3 bucket access
